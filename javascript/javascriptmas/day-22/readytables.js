const sectTable = document.getElementById('tables')
...
// more lines is better on this challenge
function displayTables() {
    // get the tables from a func
    const tables = getReadyTables()
    // mapped each number and remove comma
    const mapped = tables.map(data => {
     return `<div class="table">${data}</div>`   
    }).join('')
    // fetch tables to the DOM
    sectTable.innerHTML = mapped
}
